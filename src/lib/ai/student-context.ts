...
export async function getStudentLearningContext(supabase: SupabaseClient, userId: string) {
  ...
  const [profileResult, coursesResult, tasksResult, notesResult, sessionsResult, focusResult, examsResult] = await Promise.all([
    supabase.from("profiles").select("full_name, language, country, learning_styles, customization_settings, daily_study_goal_minutes").eq("id", userId).maybeSingle(),
    supabase.from("courses").select("name, color").eq("user_id", userId).limit(40),
    ...
    supabase.from("notes").select("title, content, updated_at").eq("user_id", userId).order("updated_at", { ascending: false }).limit(8),
    supabase.from("study_sessions").select("title, start_time, end_time, duration").eq("user_id", userId).gte("start_time", new Date().toISOString()).order("start_time", { ascending: true }).limit(30),
  ...
  return {
    ...
    courses: (coursesResult.data || []).map((course) => ({ name: course.name, color: course.color })),
    openTasks: tasksResult.data || [],
    recentNotes: (notesResult.data || []).map((note) => ({
      title: note.title,
      updated_at: note.updated_at,
      excerpt: String(note.content || "").replace(/\s+/g, " ").trim().slice(0, 700),
    })),
