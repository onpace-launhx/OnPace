...
export function PersonalizedLearningStudio({ embedded = false }: { embedded?: boolean }) {
  ...
  return <div className={`mx-auto max-w-6xl space-y-7 ${embedded ? "px-0 py-1" : "px-4 py-7 sm:px-6 lg:px-8"}`}>
    ...
    {result && <section className="space-y-4 rounded-3xl border border-brand/15 bg-white p-5 shadow-sm sm:p-6"><div><p className="text-[11px] font-extrabold uppercase tracking-[0.16em] text-brand">{t.eyebrow}</p><h2 className="mt-1 text-xl font-black text-surface-dark">{result.title}</h2></div>{result.sourceSummary && <article className="rounded-2xl bg-gray-50 p-4"><h3 className="text-xs font-extrabold text-surface-dark">{t.source}</h3><MarkdownRenderer content={result.sourceSummary} className="mt-2 text-sm leading-6 text-gray-600" /></article>}<article className="rounded-2xl border border-brand/15 bg-brand/5 p-4"><h3 className="text-xs font-extrabold text-brand">{t.answer}</h3><MarkdownRenderer content={result.directAnswer} className="mt-2 text-sm leading-6 text-surface-dark" /></article><div className="grid gap-4 md:grid-cols-2">{result.toolSections.map((section) => <article key={section.id} className="rounded-2xl border border-gray-150 p-4"><h3 className="text-xs font-extrabold text-surface-dark">{section.title}</h3><MarkdownRenderer content={section.content} className="mt-2 text-sm leading-6 text-gray-600" /></article>)}</div>{result.nextQuestion && <article className="rounded-2xl bg-violet-50 p-4"><h3 className="text-xs font-extrabold text-violet-700">{t.next}</h3><MarkdownRenderer content={result.nextQuestion} className="mt-2 text-sm font-semibold leading-6 text-violet-950" /></article>}</section>}
  </div>
}
